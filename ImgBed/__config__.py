...
enable_config = True
config_path = os.path.join(user_root, ".ImgBed_config")

questions = {
    "url": {
        "type": "input",
        "message": "ImgBed URL" if user_lang != "zh" else "图床链接",
        "default": "http[s]://<domain>:<port>/Path/to/ImgBed",
    },
    "source": {
        "type": "list",
        "message": "Source" if user_lang != "zh" else "源",
        "choices": ["Tencent Cloud", "Aliyun"],
